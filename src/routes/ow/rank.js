...
const OverwatchProfileController = require('../../controller/OverwatchProfile');
...
exports.rank = async function (req, res) {
    ...
    */
    ...
    // Obtention des stats
    const profileStats = new OverwatchProfileController(queryParameters, generateUrl(queryParameters));

    await profileStats.getProfileStats().then(result => {
        if (result.code === 200) {
            const response = profileStats.getReturnValue();
